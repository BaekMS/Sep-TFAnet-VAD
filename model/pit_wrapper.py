...
class PITLossWrapper(nn.Module):
    ...
    Examples
        ...
        mport torch
        rom asteroid.losses import pairwise_neg_sisdr
    ...
    def forward(self, est_targets, targets, target_vad=0,  return_est=False, return_incides=False, reduce_kwargs=None, **kwargs):
        ...
        if self.pit_from == "pw_mtx":
            ...
            pw_losses = self.loss_func(est_targets, targets, **kwargs) #w_losses.shape = [batch, n_src, n_src]
        ...
        #print(f"min_loss shape: {min_loss.shape}")
        #print(f"min_loss: {min_loss}")
        mean_loss = torch.mean(min_loss)
        if not return_est and not return_incides:
            #print(f"mean_loss shape: {mean_loss.shape}")
            #print(f"mean_loss: {mean_loss}")
            return mean_loss
        elif not return_est and return_incides:
            return mean_loss, batch_indices
        reordered = self.reorder_source(est_targets, batch_indices)
        if return_est and return_incides:
            return mean_loss, reordered, batch_indices 
        elif return_est and not return_incides:
            return mean_loss, reordered
        #print(f"reordered: {reordered.shape}")
        #print(f"mean_loss: {mean_loss.shape}")
    ...
    def find_best_perm_factorial(pair_wise_losses, perm_reduce=None, **kwargs):
        ...
        pwl = pair_wise_losses.transpose(-1, -2) #pw1.shape = [batch, n_src, n_src]=[btach, targets, estimates]
        perms = pwl.new_tensor(list(permutations(range(n_src))), dtype=torch.long) # perms.shape = [n_src!, n_src]
...
def reduce(perm_loss, src):
    '''
    print(f"perm_loss: {perm_loss.shape}")
    print(f"src: {src.shape}")
    print(f"nan src: {torch.sum(torch.isnan(src))}")
    print(f"nan perm_loss: {torch.sum(torch.isnan(perm_loss))}")
    '''
    #print(perm_loss.shape)
    weighted = perm_loss * torch.unsqueeze(src.norm(dim=-1, keepdim=False), dim=1) #perm_loss.shape = [batch, n_src!, n_src], src.shape = [batch, n_src, num_sample]
    #print(weighted.shape)
    return torch.mean(weighted, dim=-1) #shape = [batch, n_src!]
...
if __name__ == '__main__':
    ...
    #inmaster3
    with open(r"/dsi/gannot-lab/datasets/mordehay/data_wham_partial_overlap/test/with_wham_noise_audio/scenario_118.p", "rb") as f:
        mix_without_noise, noisy_signal, _, speakers_target, s_thetas_array = pickle.load(f)
        
    with open(r"/dsi/gannot-lab/datasets/mordehay/data_wham_partial_overlap/test/with_wham_noise_audio/scenario_119.p", "rb") as f:
        mix_without_noise2, noisy_signal2, _, speakers_target2, s_thetas_array = pickle.load(f)
    loss_func = PITLossWrapper(pairwise_neg_sisdr, pit_from='pw_mtx')
    targets = torch.tensor(speakers_target).unsqueeze(dim=0)#torch.randint(0, 100, (4,3,48000)).float()
    est_targets = torch.tensor(speakers_target2).unsqueeze(dim=0) #torch.linspace(-0.5, 0.5, 2*len(noisy_signal)).reshape(1,2,-1) #torch.randint(-1, 1, (1,2, targets.shape[-1])).float()

    #reduce_kwargs = {'src': targets}
    loss = loss_func(est_targets, targets)#, reduce_kwargs=reduce_kwargs)
